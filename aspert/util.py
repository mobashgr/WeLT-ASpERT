...
from aspert.entities import TokenSpan
...
def check_version(config, model_class, model_path):
    if os.path.exists(model_path):
        ...
        # version check
        loaded_version = config_dict.get('aspert_version', '1.0')
        if 'rel_classifier.weight' in state_dict and loaded_version != model_class.VERSION:
            msg = ("Current ASpERT version (%s) does not match the version of the loaded model (%s). "
