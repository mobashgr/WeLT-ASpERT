...
def create_train_sample(doc, neg_entity_count: int, neg_rel_count: int, max_span_size: int, rel_type_count: int):
    ...
    '''
    此处可更改
    entity_attention = torch.load('entity_attention')
    '''
    ...
    '''
    开始：如果不足，补足100个实体关系对
    if len(neg_rel_spans+pos_rel_spans) < 100:
        entity_spans = pos_entity_spans + list(neg_entity_spans)
        for i1, s1 in enumerate(entity_spans):
            for i2, s2 in enumerate(entity_spans):
                if s1 not in pos_entity_spans and s2 not in pos_entity_spans:
                    if s1 != s2:
                        neg_rel_spans.append((s1, s2))
        neg_rel_spans = random.sample(neg_rel_spans, min(len(neg_rel_spans), neg_rel_count))
        neg_rels = [(entity_spans.index(s1), entity_spans.index(s2)) for s1, s2 in neg_rel_spans]
